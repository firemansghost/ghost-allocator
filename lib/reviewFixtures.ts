/**
 * Review Fixtures
 * Deterministic test cases for builder output review harness
 */

import type { QuestionnaireAnswers, RiskLevel } from './types';

export interface ReviewFixture {
  id: string;
  title: string;
  notes?: string;
  answers: QuestionnaireAnswers;
  expectedRiskLevel: RiskLevel;
}

/**
 * Review Fixtures
 * These are deterministic test cases used by the review harness
 */
export const REVIEW_FIXTURES: ReviewFixture[] = [
  {
    id: 'voya-only-standard-moderate',
    title: 'Voya-only + Standard + Moderate Risk',
    notes: 'Baseline: Voya-only path with standard preset',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_only',
      portfolioPreset: 'standard',
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-schwab-standard-moderate',
    title: 'Voya+Schwab + Standard + Moderate Risk',
    notes: 'Baseline: Voya+Schwab path with standard preset',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'standard',
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-schwab-house-60-30-10-moderate',
    title: 'Voya+Schwab + House 60/30/10 + Moderate Risk',
    notes: 'House preset: 60% SPYM, 30% GLDM, 10% FBTC',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'ghostregime_60_30_10',
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-schwab-house-60-25-15-moderate',
    title: 'Voya+Schwab + House 60/25/15 + Moderate Risk',
    notes: 'House preset: 60% SPYM, 25% GLDM, 15% FBTC',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'ghostregime_60_25_15',
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-schwab-house-60-30-10-aggressive',
    title: 'Voya+Schwab + House 60/30/10 + Aggressive Risk',
    notes: 'House preset with higher risk level (should affect Voya defensive mix)',
    answers: {
      age: 35,
      yearsToGoal: 30,
      isRetired: false,
      drawdownTolerance: 'high',
      behaviorInCrash: 'buy_more',
      incomeStability: 'high',
      complexityPreference: 'advanced',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 60,
      schwabPreference: 'use_full_75',
      portfolioPreset: 'ghostregime_60_30_10',
    },
    expectedRiskLevel: 5,
  },
  {
    id: 'voya-schwab-standard-tilt-moderate',
    title: 'Voya+Schwab + Standard + Tilt 10/5 + Moderate Risk',
    notes: 'Standard preset with Gold 10% / Bitcoin 5% tilt',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'standard',
      goldBtcTilt: 'gold10_btc5',
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-schwab-standard-simplify-moderate',
    title: 'Voya+Schwab + Standard + Simplify + Moderate Risk',
    notes: 'Standard preset with Simplify mode lineup',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'standard',
      schwabLineupStyle: 'simplify',
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-schwab-standard-simplify-wrappers-moderate',
    title: 'Voya+Schwab + Standard + Simplify + Wrappers + Moderate Risk',
    notes: 'Simplify mode with YGLD/MAXI income wrappers',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'standard',
      schwabLineupStyle: 'simplify',
      goldInstrument: 'ygld',
      btcInstrument: 'maxi',
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-schwab-house-wrappers-moderate',
    title: 'Voya+Schwab + House 60/30/10 + Wrappers + Moderate Risk',
    notes: 'House preset with YGLD/MAXI income wrappers',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'ghostregime_60_30_10',
      goldInstrument: 'ygld',
      btcInstrument: 'maxi',
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-only-simple-tdf-current',
    title: 'Voya-only + Simple + TDF Current Holdings + Moderate Risk',
    notes: 'User has 100% in target-date fund as current holdings; should recommend non-TDF mix',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'simple',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_only',
      portfolioPreset: 'standard',
      currentVoyaHoldings: [
        {
          fundId: 'vanguard_target_retirement_2035',
          fundName: 'Vanguard Target Retirement 2035',
          allocationPct: 100,
        },
      ],
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-schwab-house-scaled-moderate',
    title: 'Voya+Schwab + House Preset + GhostRegime Scaling + Moderate Risk',
    notes: 'House preset with mocked GhostRegime scales (stocks_scale=0.5, gold_scale=1, btc_scale=0) to test scaled lineup',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'ghostregime_60_30_10',
      goldInstrument: 'gldm',
      btcInstrument: 'fbtc',
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-only-template-conservative',
    title: 'Voya-only + Template Conservative (Risk 2)',
    notes: 'Template: Conservative with riskLevelOverride=2',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_only',
      portfolioPreset: 'standard',
      riskLevelOverride: 2,
    },
    expectedRiskLevel: 2,
  },
  {
    id: 'voya-only-template-balanced',
    title: 'Voya-only + Template Balanced (Risk 3)',
    notes: 'Template: Balanced with riskLevelOverride=3',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_only',
      portfolioPreset: 'standard',
      riskLevelOverride: 3,
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-only-template-growth',
    title: 'Voya-only + Template Growth (Risk 4)',
    notes: 'Template: Growth with riskLevelOverride=4',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_only',
      portfolioPreset: 'standard',
      riskLevelOverride: 4,
    },
    expectedRiskLevel: 4,
  },
  {
    id: 'voya-only-template-aggressive',
    title: 'Voya-only + Template Aggressive (Risk 5)',
    notes: 'Template: Aggressive with riskLevelOverride=5',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_only',
      portfolioPreset: 'standard',
      riskLevelOverride: 5,
    },
    expectedRiskLevel: 5,
  },
  {
    id: 'voya-schwab-template-conservative',
    title: 'Voya+Schwab + Template Conservative (Risk 2)',
    notes: 'Template: Conservative with riskLevelOverride=2',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'standard',
      riskLevelOverride: 2,
    },
    expectedRiskLevel: 2,
  },
  {
    id: 'voya-schwab-template-balanced',
    title: 'Voya+Schwab + Template Balanced (Risk 3)',
    notes: 'Template: Balanced with riskLevelOverride=3',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'standard',
      riskLevelOverride: 3,
    },
    expectedRiskLevel: 3,
  },
  {
    id: 'voya-schwab-template-growth',
    title: 'Voya+Schwab + Template Growth (Risk 4)',
    notes: 'Template: Growth with riskLevelOverride=4',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'standard',
      riskLevelOverride: 4,
    },
    expectedRiskLevel: 4,
  },
  {
    id: 'voya-schwab-template-aggressive',
    title: 'Voya+Schwab + Template Aggressive (Risk 5)',
    notes: 'Template: Aggressive with riskLevelOverride=5',
    answers: {
      age: 45,
      yearsToGoal: 20,
      isRetired: false,
      drawdownTolerance: 'medium',
      behaviorInCrash: 'hold',
      incomeStability: 'medium',
      complexityPreference: 'moderate',
      hasPension: false,
      pensionCoverage: 'none',
      platform: 'voya_and_schwab',
      currentSchwabPct: 50,
      schwabPreference: 'stay_low',
      portfolioPreset: 'standard',
      riskLevelOverride: 5,
    },
    expectedRiskLevel: 5,
  },
];

