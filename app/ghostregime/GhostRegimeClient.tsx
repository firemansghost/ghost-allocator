/**
 * GhostRegime Client Component
 * Displays today's regime classification and allocations.
 * Receives initial health + today from SSR for immediate first paint.
 */
...
export interface GhostRegimeClientProps {
  initialHealth: HealthStatus | null;
  initialToday: GhostRegimeRow | null;
  initialNotSeeded?: boolean;
  initialError?: string | null;
}

export function GhostRegimeClient({
  initialHealth,
  initialToday,
  initialNotSeeded = false,
  initialError = null,
}: GhostRegimeClientProps) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const [data, setData] = useState<GhostRegimeRow | null>(initialToday);
  const [loading, setLoading] = useState(
    !initialToday && !initialNotSeeded && !initialError
  );
  const [error, setError] = useState<string | null>(initialError);
  const [notSeeded, setNotSeeded] = useState(initialNotSeeded);
  const [showAdvanced, setShowAdvanced] = useState(false);
  const [healthStatus, setHealthStatus] = useState<HealthStatus | null>(initialHealth);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // SSR: we have error/notSeeded state — skip fetch
      if (!asof && !prev && (initialNotSeeded || initialError)) {
        setLoading(false);
        return;
      }

      try {
        // Fetch health status (only when we need to fetch)
    ...
    fetchData();
  }, [searchParams, initialToday, initialNotSeeded, initialError]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          {/* Risk Regime Chip — secondary */}
          <div className="inline-flex items-center gap-2 px-2.5 py-1.5 rounded-md border border-zinc-800/80 bg-zinc-900/40">
            <span className="text-[10px] text-zinc-500 uppercase tracking-wide">Risk</span>
            <span className="text-xs font-medium text-zinc-400">{data.risk_regime}</span>
          </div>
        </div>
      </div>
        ...
        <GlassCard className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link
                ...
                className="text-[11px] text-zinc-500 hover:text-amber-400/85 underline underline-offset-2"
              >
                {REGIME_MAP_METHODOLOGY_CTA}
              </Link>
            </div>
