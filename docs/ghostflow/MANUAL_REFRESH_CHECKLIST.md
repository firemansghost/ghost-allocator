...
**Treasury Plumbing:** Two production candidates (v1.7d–d.1) displayed in separate UI lane (v1.7e) — **not scored**, not in `publicSignalCount`, no equity composite merge ([mapping decision](./TREASURY_PLUMBING_MAPPING_DECISION.md)). Artifact refresh updates the display-only Treasury Plumbing lane only; **no score impact** and **no** `publicSignalCount` change. Weekly CFTC: [`treasuryFuturesPositioningProxy.v1.json`](../data/ghostflow/artifacts/treasuryFuturesPositioningProxy.v1.json). Daily FRED: [`treasuryLongEndIncomeLens.v1.json`](../data/ghostflow/artifacts/treasuryLongEndIncomeLens.v1.json) — six-series common-date extract ([BOND_NEGLECT_INCOME_LENS_ARTIFACT_DESIGN.md](./BOND_NEGLECT_INCOME_LENS_ARTIFACT_DESIGN.md)); **no forward-fill**. UI caveat: futures proxy is not full basis-trade measurement; income lens is not investment advice.
