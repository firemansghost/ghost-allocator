...
| **v1.9e.2** | Tail Skew Context Artifact Design | **Done** — [TAIL_SKEW_CONTEXT_ARTIFACT_DESIGN.md](./TAIL_SKEW_CONTEXT_ARTIFACT_DESIGN.md); docs-only; SKEW-only display-only schema; no JSON/UI/score/runtime; `publicSignalCount` **12** unchanged; next: **v1.9e.3** example artifact + validator/types/tests |
