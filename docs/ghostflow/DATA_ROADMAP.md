...
| **v1.7c** | Bond Neglect / Long-End Income Lens artifact design | **Done** — [BOND_NEGLECT_INCOME_LENS_ARTIFACT_DESIGN.md](./BOND_NEGLECT_INCOME_LENS_ARTIFACT_DESIGN.md); `treasury-long-end-income-lens` example + validator/tests; FRED IDs candidate until v1.7d; no production/display/score |
...
6. **Treasury Plumbing (v1.7a–c):** [TREASURY_PLUMBING_FEASIBILITY.md](./TREASURY_PLUMBING_FEASIBILITY.md) · [TREASURY_BASIS_TRADE_ARTIFACT_DESIGN.md](./TREASURY_BASIS_TRADE_ARTIFACT_DESIGN.md) · [BOND_NEGLECT_INCOME_LENS_ARTIFACT_DESIGN.md](./BOND_NEGLECT_INCOME_LENS_ARTIFACT_DESIGN.md) — CFTC **GREEN** (v1.7a.1); both Treasury artifact designs **complete** (positioning + long-end income lens). Open: **FRED operator extract / field lock** → **v1.7d** production JSON; **v1.7e** display section. Treasury lane **separate from equity composite**; `publicSignalCount` **10** unchanged.
