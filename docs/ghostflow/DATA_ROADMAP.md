...
| **v1.9c.4** | Index Inclusion Event Production Artifact + Display Card | **Deferred** — product-gated; **blocker:** operator-verified event data required; likely `publicSignalCount` 10 → 11 when approved |
...
| Passive Supply / Float Absorption Lens | **v1.9c.4a Done** · **v1.9c.4 deferred** | Core — [feasibility](./PASSIVE_SUPPLY_FLOAT_ABSORPTION_FEASIBILITY.md) · [source spike](./PASSIVE_SUPPLY_SOURCE_SPIKE.md) · [artifact design](./PASSIVE_SUPPLY_EVENT_ARTIFACT_DESIGN.md) |
...
**Not approved:** production artifact JSON, UI cards, runtime feeds, `publicSignalCount` changes, or score gates. v1.9c.3 example/validator scaffolding complete; v1.9c.4a operator provenance checklist complete; v1.9c.4 production/display **deferred** — blocked on operator-verified event rows.
