...
function printDryRun(plan: ReturnType<typeof buildDryRunPlan>): void {
  ...
  console.log(`Cap-weight min rows (default): ${plan.minRowsRequired}`);
  console.log('');
  ...
  console.log('');
  console.log(`Warning: ${plan.coverageWarning}`);
  console.log('');
...
async function main(): Promise<void> {
  ...
  console.log(`Coverage gate: ≥${args.minRowsRequired} rows and last date within ${args.maxEndDateLagDays} days of date-to`);
  ...
  for (const symbol of args.symbols) {
    ...
    const coverage = assessExportCoverage({
      ...
      maxEndDateLagDays: args.maxEndDateLagDays,
    });

    if (coverage.coverageStatus === 'partial') {
      partialSymbols.push(symbol);
      console.error(formatCoverageFailure(symbol, coverage));
      if (!args.allowPartial) {
        console.error('');
        console.error(`Export aborted for ${symbol} (incomplete coverage). Pass --allow-partial to write exploratory output.`);
        console.error(`API calls used: ${totalApiCalls}`);
        process.exit(1);
      }
      console.error(`--allow-partial set: writing ${symbol} with coverageStatus=partial`);
    }
    ...
    console.log(
      `  Wrote ${csvPath} (${result.rows.length} rows, ${first} → ${last}, ${result.apiCalls} API call(s), coverage=${coverage.coverageStatus})`
    );
  ...
  if (partialSymbols.length > 0) {
    console.error('');
    console.error(
      `Partial coverage for: ${partialSymbols.join(', ')}. Output is NOT sufficient for cap-weight study or artifact refresh.`
    );
    console.error('Use Yahoo/manual adjusted-close CSVs for production.');
    process.exit(2);
  }
